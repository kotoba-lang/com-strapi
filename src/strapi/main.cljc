(ns strapi.main
  "Kotodama WASM entrypoint for the Strapi clean-room actor (L4) — Clojure port.
  ...
  facts (`strapi.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "strapi")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"strapi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "strapi-compat" :tier tier :entities entities} 200])
...
(defn start! [] :strapi-compat/ready)
